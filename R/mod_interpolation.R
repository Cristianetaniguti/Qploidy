...
mod_interpolation_ui <- function(id){
  ...
  tagList(
    fixedRow(
      column(12,
             h3("Upload allele intensities or counts"),
    ...
    mainPanel(
      tabsetPanel(
        tabPanel("Tables",
                 hr(),
                 DT::dataTableOutput(ns("filtered_markers"))
        ),
        tabPanel("Graphics",
                 hr(),
...
mod_interpolation_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    lst_interpolation <- reactive({
      ...
      return(list(lst_interpolation, missing.data))
    })
    ...
    observe({
      mks <- names(clusters())
      choices <- as.list(mks)
      mks[which(filters()[[3]])] <- paste(mks[which(filters()[[3]])], "(discarded)")
      names(choices) <- mks
    ...
    output$interpolation <- renderPlot({
      plot_one_marker(lst_interpolation()[[1]][[input$int_marker]], ploidy = input$ploidy)
    })
