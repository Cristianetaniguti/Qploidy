  ...
  for(j in 1:length(ploidys)){
    ...
    for(z in 1:length(by_chr)){
      for(i in 1:nrow(rets)){
        ...
        modes <- apply(by_chr[[z]][,-c(1,2)], 2, function(x) mode(x[x >= rets$ymin[i] & x <= rets$ymax[i]],na.rm = TRUE))
...
plot_baf <- function(data_sample,
  ...
  if(add_estimated_peaks | add_expected_peaks){
  ...
  p_baf <- data_sample %>% ggplot(aes(x=Position, y=sample)) +
    ...
    {if(add_expected_peaks) geom_rect(data = rets, inherit.aes=FALSE,
...
plot_baf_hist <- function(data_sample,
  ...
  if(add_estimated_peaks | add_expected_peaks | colors){
    ...
    for(i in 1:nrow(rets)) {
      ...
      modes.df2[[i]] <- matrix(unlist(sapply(split_data, function(x){
        estimated <- mode(x[which(x > rets$ymin[i] & x < rets$ymax[i])]) #!
        expected <- seq(0, 1, 1/ploidy)[i]
        return(data.frame(estimated, expected))
      })), nrow = 2)
      colnames(modes.df2[[i]]) <- names(split_data)
      rownames(modes.df2[[i]]) <- c("peaks estimated", "peaks expected")
    }
    ...
    modes.df2$alpha <- gsub("peaks.expected", 1, modes.df2$alpha)
    modes.df2$alpha <- as.numeric(gsub("peaks.estimated", 0.5, modes.df2$alpha))

    data_sample$color <- NA
    data_sample$color[which(!idx_tot)] <- "inside area"
    data_sample$color[which(idx_tot)] <- "outside area"
  }
  ...
  p_hist <- data_sample %>% ggplot(aes(x=sample)) +
    ...
    {if(add_estimated_peaks) geom_vline(data = modes.df2[which(modes.df2$name == "peaks.estimated"),],
                                        aes(xintercept= value,
                                            color = name,
                                            linetype = name,
                                            alpha = alpha),
                                        linewidth = 0.8)}+
    {if(add_expected_peaks) geom_vline(data = modes.df2[which(modes.df2$name == "peaks.expected"),],
                                       aes(xintercept= value,
                                           color = name,
                                           linetype = name,
                                           alpha = alpha),
