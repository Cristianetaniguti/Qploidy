...
saveRDS(est.ploidy.chr_df,  file = "ploidy.data.frames_2-5.rds")
...
genoprob <- read_polyOrigin("C:/Users/Rose_Lab/Documents/Cris_temp/Qploidy_data/roses_texas/5pop_compareprob2_fulldata_polyancestry.csv")

f1.codes <- vroom(system.file("F1codes.polyorigin.txt", package = "Qploidy"), show_col_types = FALSE)
haplo_polyorigin <- get_probs_polyorigin(genoprob,
                                         f1.codes = f1.codes,
                                         ploidy = 4, n.cores = 20)
...
vroom_write(p_p_df, file = "polyorigin_break_counts.txt")

p_p_df <- vroom("polyorigin_break_counts.txt")
end <- c(seq(20,140, 20), 161) 
start <- seq(1,141, 20)

p_p_plot <- list()
for(i in 1:length(end)){
  temp <- p_p_df[which(p_p_df$individual %in% unique(p_p_df$individual)[start[i]:end[i]]),]
  p_p_plot[[i]] <- count_breaks_plot(temp, by_LG = FALSE)
  ggsave(p_p_plot[[i]], file= paste0("p_polyorigin_counts_", i, ".png"), height = 8, width = 5)
}
...
write.csv(new_all, file = "new_all7.csv")
