...
Here, we will demonstrate the use of a simulated VCF file and present results from an actual rose Axiom array dataset. For more details about the rose dataset, please refer to this [publication]().
...
simulate_vcf(
  ...
  n_tetraploid = 35, n_diploid = 5, n_triploid = 10, n_markers = 500
)
```
...
4   AX-86752740 High_Voltage.1 1368.1266  645.9481 2014.075 0.3207170
6   AX-86752740 Morden_Blush.1  302.1575 1542.4037 1844.561 0.8361900
104 AX-86752740 Morden_Blush.2  304.3137 1536.8583 1841.172 0.8347174
```
...
genos.pos <- data.frame(
  MarkerName = multidog_obj$snpdf$snp,
...
```{r, eval=FALSE}
roses_data_standardized <- standardize(
  data = data,
  genos = genos,
  geno.pos = genos.pos,
...
p <- plot_qploidy_standardization(
  x = roses_data_standardized,
  sample = sample,
  type = c("het", "BAF", "zscore"),
  dot.size = 0.05,
  chr = 2:8
)
ggsave(p, filename = "fig1.jpg")
```
...
p <- plot_qploidy_standardization(
  ...
  centromeres = c(
    "1" = 22000000, "2" = 36000000, "3" = 4000000,
    "4" = 20000000, "5" = 52000000, "6" = 32000000, "7" = 20000000
  )
)

ggsave(p, filename = "fig2.jpg")
```
...
estimated_ploidies_sample <- area_estimate_ploidy(
  qploidy_standardization = roses_data_standardized, # standardization result object
  samples = "all", # Samples or "all" to estimate all samples
  level = "sample", # Resolution level
  ploidies = c(2, 5)
) # Ploidy range to investigate
...
estimated_ploidies_chromosome <- area_estimate_ploidy(
  qploidy_standardization = roses_data_standardized,
  samples = "all",
  level = "chromosome",
  ploidies = c(2, 5)
)
...
for (i in 1:length(samples)) {
  ...
  all_resolutions_plots(
    data_standardized = roses_data_standardized,
    sample = samples[i],
...
# iii) Run standardization again using this subset as the reference.
roses_data_standardized_round2 <- standardize(
  data = data,
  genos = genos_round2,
  geno.pos = genos.pos,
  ploidy.standardization = 4,
...
# Proportion of heterozygous loci, BAF (Qploidy standardized ratio), and zscore by genomic positions
p <- plot_qploidy_standardization(
  x = roses_data_standardized_round2,
  sample = sample,
  type = c("het", "BAF", "zscore"),
  dot.size = 0.05,
  chr = 2:8,
...
# If chromosome-arm resolution
estimated_ploidies_chromosome_arm <- area_estimate_ploidy(
  qploidy_standardization = roses_data_standardized_round2,
  samples = "all",
  level = "chromosome-arm",
  ploidies = c(2, 5),
  centromeres = c(
...
for (i in 1:length(samples)) {
  ...
  # This function will generate figures for sample, chromosome and chromosome-arm resolution level evaluation
  all_resolutions_plots(
    data_standardized = roses_data_standardized_round2,
    sample = samples[i],
    ploidy = estimated_ploidies_chromosome$ploidy[i, 2:8],
    chr = 2:8,
    centromeres = c(
